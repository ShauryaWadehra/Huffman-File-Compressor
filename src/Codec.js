...
export default class Codec {
	...
	encode(data) {
		...
		if (mp.size === 0) {
			...
			let output_message = "Compression completed and file will be downloaded automatically." + '\n' + "Compression Ratio : " + (data.length / final_string.length).toPrecision(6);
	...
	decode(data) {
		...
		if (temp === "zer") {
			let decoded_data = "";
			let output_message = "De-Compression completed and file will be downloaded automatically.";
...
export function myDownloadFile(fileName, text) {
